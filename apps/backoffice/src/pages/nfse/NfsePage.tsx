...
export function NfsePage() {
  ...
  const statusBadgeClass = (s: string | null) =>
    s === 'authorized' ? 'badge-product' : s === 'rejected' ? 'badge-service' : 'badge-raw_material';
            ...
            <tbody>
              {items.map(n => (
                <tr key={n.id} style={{ cursor: 'pointer' }} onClick={() => openDetail(n)}>
                  ...
                  <td>
                    <span className={`badge ${statusBadgeClass(n.nfse_status)}`}>{statusLabel(n.nfse_status)}</span>
                  </td>
        ...
        <div className="overlay" onClick={() => setDrawerOpen(false)}>
          <div className="drawer" onClick={e => e.stopPropagation()}>
                ...
                <>
                  ...
                  <div className="card" style={{ padding: 16, marginBottom: 16 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span className={`badge ${statusBadgeClass(detail.nfse_status)}`}>{statusLabel(detail.nfse_status)}</span>
                      {detail.nfse_number && (
                  ...
                  {detail.nfse_pdf_url && (
                    <div style={{ margin: '12px 0' }}>
                      <a className="btn btn-secondary btn-sm" href={detail.nfse_pdf_url} target="_blank" rel="noreferrer">
                        {t('nfse.viewPdf')}
                      </a>
                    </div>
                  )}
                  ...
                  {detail.events.length === 0 ? (
                    ...
                    <table style={{ fontSize: 12 }}>
                      <thead>
                        <tr><th>{t('nfse.event')}</th><th>{t('nfse.code')}</th><th>{t('nfse.date')}</th></tr>
                      </thead>
                      <tbody>
            ...
            <div className="drawer-footer">
              {detail && detail.nfse_status === 'rejected' && (
