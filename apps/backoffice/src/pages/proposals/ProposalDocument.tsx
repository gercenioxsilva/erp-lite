...
import { digits, maskCNPJ, maskCPF, normalizeCNPJ } from '../../lib/brazil';

/* ── Types (compartilhados entre portal público e impressão interna) ─────── */
...
export interface IssuerData extends PartyData { logo_url: string | null; banner_url: string | null; }
...
export const paymentLabel = (k: string | null): string => (k ? (PAYMENT_LABELS[k] ?? k) : '');

/**
 * Formata CNPJ (inclusive alfanumérico — IN RFB nº 2.229/2024) e CPF usando as
 * funções canônicas de apps/backoffice/src/lib/brazil.ts — nunca reimplementar
 * a máscara localmente (regra 36 do README, protocolo anti-alucinação).
 */
export function fmtDoc(doc: string | null, type: string | null): string {
  if (!doc) return '—';
  if (type === 'CPF') return maskCPF(doc);
  if (type === 'CNPJ') return maskCNPJ(doc);
  // Tipo desconhecido: tenta CNPJ (14 posições) e cai para CPF (11 dígitos).
  if (normalizeCNPJ(doc).length === 14) return maskCNPJ(doc);
  if (digits(doc).length === 11) return maskCPF(doc);
  return doc;
}
...
export function addressLines(o: PartyData): string[] {
...
export const IconCalendar = () => (<svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4M8 2v4M3 10h18"/></svg>);

/* ── Scoped stylesheet (fonts + layout + print) ─────────────────────────── */
export const PROPOSAL_DOCUMENT_STYLES = `
...
export function ProposalDocumentStyle() {
  return <style dangerouslySetInnerHTML={{ __html: PROPOSAL_DOCUMENT_STYLES }} />;
}

/* ── Document (card + footer) — layout idêntico ao portal público /p/:token ── */
...
export function ProposalDocument({ proposal, items, issuer, client, clientName, pill, banner, children }: ProposalDocumentProps) {
  const hasDiscountCol = items.some(it => it.discount_pct > 0);
  const issuerAddr = addressLines(issuer);
  const name = clientName ?? client?.name ?? null;
      ...
      <div className="pp-card">
        ...
        <div className="pp-meta">
          <div className="pp-meta__to">
            ...
            <div className="pp-meta__v">{name || 'Cliente'}</div>
          </div>
