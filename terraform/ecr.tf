...
resource "aws_ecr_repository" "lambda_whatsapp" {
  name                 = "erp-lite/lambda-whatsapp"
  image_tag_mutability = "MUTABLE"
  image_scanning_configuration { scan_on_push = true }
  tags = { Environment = var.environment }
}

resource "aws_ecr_lifecycle_policy" "lambda_whatsapp" {
  repository = aws_ecr_repository.lambda_whatsapp.name
  policy     = local.ecr_lifecycle_policy
}
