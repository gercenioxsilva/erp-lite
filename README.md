...
25. **CEP automático via ViaCEP — nunca criar endpoint backend para isso.** A função `fetchAddressByCEP(cep)` já existe em `apps/backoffice/src/lib/brazil.ts`. Ela chama `https://viacep.com.br/ws/{cep}/json/` diretamente do browser (CORS habilitado, gratuito, sem API key). Uso correto: `onBlur` no campo CEP, só disparar quando `digits(cep).length === 8`, preencher os campos de endereço sem sobrescrever campos editados pelo usuário. Nunca criar uma rota `/v1/cep/:cep` no backend.
...
> **Módulo de fornecedores integrado às Contas a Pagar:**
> Tabela `suppliers` com dados completos (PJ/PF, endereço, dados bancários, chave PIX,
> categoria). `payables` ganha `supplier_id UUID FK → suppliers ON DELETE SET NULL`
> retrocompatível — `supplier_name` (texto livre) permanece como fallback para contas avulsas.
>
> **Migration:** `0020_suppliers.sql` — cria `suppliers`, adiciona `supplier_id` em `payables`.
>
> **API:** CRUD em `/v1/suppliers` + `GET /v1/suppliers/:id/payables`.
> `POST/PATCH /v1/payables` aceita `supplier_id` e resolve `supplier_name` automaticamente.
> `GET /v1/payables` retorna nome via LEFT JOIN com `suppliers`.
>
> **Frontend:** `SuppliersPage.tsx` (lista + drawer 2 abas: Dados Gerais / Dados Bancários).
> `PayablesPage.tsx`: campo fornecedor vira select de fornecedores cadastrados + input texto livre.
> CEP com preenchimento automático via `fetchAddressByCEP` no `SuppliersPage`.
>
> **CEP em CompanyPage:** campo CEP da aba Dados Fiscais (nfe-config) ganhou `onBlur`
> que chama `fetchAddressByCEP` e preenche `logradouro`, `bairro`, `municipio`, `uf` automaticamente.
> `ClientsPage` já tinha essa integração desde v1.0.
>
> **CEP:** zero infra nova — `fetchAddressByCEP` em `brazil.ts` chama ViaCEP diretamente do browser.
> Gratuito, sem API key, CORS habilitado.
>
> **Protocolo anti-alucinação:** regra 1 atualizada com `suppliers`; regra 25 (CEP via ViaCEP) adicionada.

### v10.0 — Reset de Senha + XLSX Export + Dashboard KPIs + Contas Recorrentes + Notificações de Vencimento

> **5 funcionalidades adicionadas ao ERP em um único release:**
>
> **1. Reset de Senha via E-mail:**
> Fluxo completo de recuperação de senha sem expor existência de e-mails.
> `POST /v1/auth/forgot-password` — busca usuário, gera token 32-char hex, expira em 2h,
> envia e-mail com link via SQS→lambda-notifications (fire-and-forget), **sempre retorna `{ ok: true }`**.
> `POST /v1/auth/reset-password` — valida token+expiry, atualiza `password_hash` (bcrypt rounds=12),
> limpa token. Frontend: `ForgotPasswordPage.tsx`, `ResetPasswordPage.tsx`, link "Esqueceu a senha?"
> na `LoginPage.tsx`. Migrations: `0021_users_password_reset.sql` (`password_reset_token`, `password_reset_expires`).
>
> **2. Exportação XLSX em 4 módulos:**
> Botão "Exportar" em `ReceivablesPage`, `PayablesPage`, `InvoicesPage`, `ClientsPage`.
> Exportação client-side via SheetJS (`xlsx`) — zero mudanças no backend. Campos selecionados
> por módulo com filename datado (ex: `contas-a-receber-2026-06-24.xlsx`).
>
> **3. Dashboard Financeiro com KPIs:**
> Novo endpoint `GET /v1/dashboard` com 5 queries paralelas via `Promise.all`:
> receivables pendentes/vencidas, payables da semana, revenue do mês vs. mês anterior,
> pedidos confirmados, receita por mês (últimos 6 meses). `DashboardPage.tsx` reescrito
> com bento grid de cards e mini-gráfico de barras CSS puro (sem biblioteca externa).
>
> **4. Contas a Pagar Recorrentes:**
> Colunas `recurrence` (none/weekly/monthly/quarterly/yearly), `recurrence_day`, `recurrence_end_date`,
> `recurrence_last_generated`, `parent_payable_id` (FK self-referencial) em `payables`.
> Worker `recurringPayablesWorker.ts` em-processo no ECS: a cada 23h, gera próxima instância
> quando `recurrence != 'none'`. Frontend: select de recorrência + campo dia + data fim no
> formulário de payables; badge `↻ Mensal` etc. na tabela. Migration: `0022_payables_recurrence.sql`.
>
> **5. Notificações de Vencimento de Recebíveis:**
> Coluna `notify_receivable_due_days SMALLINT DEFAULT 3` em `notification_configs`;
> coluna `due_notification_sent BOOLEAN DEFAULT false` em `receivables`.
> Worker `dueSoonWorker.ts` em-processo no ECS: a cada 23h, envia SQS `receivable_due_soon`
> para recebíveis que vencem em N dias (configurável por tenant). Template HTML em
> `lambda-notifications`. Frontend: select "Notificar vencimento X dias antes" na aba
> Notificações da `CompanyPage.tsx`. Migration: `0023_notification_due.sql`.
>
> **Protocolo anti-alucinação:** regra 1 atualizada com novas colunas; regra 23 atualizada
> com tipo `password_reset`; regra 26 (workers ECS in-process) adicionada.
...
| password_reset_token | VARCHAR(255) | *(0021)* Token de 32-char hex para reset de senha. `NULL` quando não há reset pendente. |
| password_reset_expires | TIMESTAMPTZ | *(0021)* Data de expiração do token (2h após geração). `NULL` quando sem reset pendente. |
...
### `notification_configs` *(migration: 0010_notification_configs.sql + 0019_nfse.sql + 0023_notification_due.sql)*
| Campo | Tipo | Notas |
|-------|------|-------|
...
| notify_receivable_due_days | SMALLINT DEFAULT 3 | *(0023)* Dias de antecedência para notificar vencimento de recebíveis. `0` = desabilitado. |
| created_at / updated_at | TIMESTAMPTZ | |
...
| due_notification_sent | BOOLEAN DEFAULT false | *(0023)* Controla idempotência do worker `dueSoonWorker` — `true` após envio da notificação de vencimento próximo. |
| notes | TEXT | |
| created_at / updated_at | TIMESTAMPTZ | |
...
### `suppliers` *(migration: 0020_suppliers.sql)*
Cadastro de fornecedores por tenant. Vinculado a `payables` via `supplier_id` (opcional).

| Campo | Tipo | Notas |
|-------|------|-------|
| id | UUID PK | |
| tenant_id | UUID FK → tenants ON DELETE CASCADE | |
| person_type | VARCHAR(2) NOT NULL | `PJ`\|`PF` |
| company_name | VARCHAR(255) | Razão social — obrigatório se PJ |
| trade_name | VARCHAR(255) | Nome fantasia |
| cnpj | VARCHAR(14) | 14 dígitos sem máscara |
| full_name | VARCHAR(255) | Nome completo — obrigatório se PF |
| cpf | VARCHAR(11) | 11 dígitos sem máscara |
| email | VARCHAR(255) | |
| phone | VARCHAR(30) | |
| zip_code | VARCHAR(8) | 8 dígitos sem hífen |
| street / street_number / complement / neighborhood / city / state | VARCHAR | Endereço completo |
| bank_code / agency / account / account_digit | VARCHAR | Dados bancários para pagamento |
| pix_key | VARCHAR(255) | Chave PIX (retornada mascarada na API) |
| category | VARCHAR(50) NOT NULL DEFAULT 'services' | `services`\|`supplies`\|`utilities`\|`rent`\|`payroll`\|`taxes`\|`other` |
| notes | TEXT | |
| is_active | BOOLEAN DEFAULT true | Soft-delete |
| created_at / updated_at | TIMESTAMPTZ | |

### `payables` *(migration: 0013_payables.sql + 0022_payables_recurrence.sql)*
...
| id | UUID PK | |
| tenant_id | UUID FK → tenants | |
| supplier_id | UUID FK → suppliers ON DELETE SET NULL | Fornecedor cadastrado (opcional — `supplier_name` permanece como fallback) |
| supplier_name | VARCHAR(255) | Nome do fornecedor (texto livre — fallback quando supplier_id é NULL) |
| description | VARCHAR(500) NOT NULL | |
...
| status | VARCHAR(20) | `pending`→`partial`→`paid`\|`overdue`\|`cancelled` |
| recurrence | VARCHAR(20) DEFAULT 'none' | *(0022)* `none`\|`weekly`\|`monthly`\|`quarterly`\|`yearly` |
| recurrence_day | SMALLINT | *(0022)* Dia do mês (1-31) para recorrência monthly/quarterly/yearly |
| recurrence_end_date | DATE | *(0022)* Data final da série recorrente (opcional) |
| recurrence_last_generated | DATE | *(0022)* Controla idempotência do worker — data da última geração |
| parent_payable_id | UUID FK → payables ON DELETE SET NULL | *(0022)* Referência à conta original da série (FK auto-referencial) |
| notes | TEXT | |
| created_at / updated_at | TIMESTAMPTZ | |
...
| POST | `/v1/auth/forgot-password` | Solicitar reset de senha — **sempre retorna `{ ok: true }`** mesmo se e-mail não encontrado (anti-enumeração) |
| POST | `/v1/auth/reset-password` | Redefinir senha usando token (`{ token, password }`) — retorna `{ ok: true }` ou 400 se token inválido/expirado |

**Body de forgot-password:** `{ "email": "user@example.com" }`

**Body de reset-password:** `{ "token": "32charHexToken", "password": "minLength6" }`

**Segurança:**
- Token expira em 2 horas
- `POST /forgot-password` nunca revela se o e-mail existe (evita enumeração de usuários)
- Após reset bem-sucedido, o token é apagado do banco (uso único)
- Envio do e-mail é fire-and-forget — falhas de e-mail não retornam erro para o cliente
...
```json
{
  ...
  "notify_receivable_due_days": 3
}
```
- `notify_receivable_due_days`: dias de antecedência para notificar vencimento de recebíveis. `0` = desabilitado. Opções: `0`, `1`, `3`, `5`, `7`.

### Dashboard (KPIs Financeiros)
| Método | Rota | Descrição |
|--------|------|-----------|
| GET | `/v1/dashboard` | KPIs financeiros do tenant autenticado (requer Bearer) |

**Response:**
```json
{
  "receivables": {
    "pending_count": 12, "pending_amount": 48500.00,
    "overdue_count": 3,  "overdue_amount": 9200.00
  },
  "payables": {
    "due_week_count": 5, "due_week_amount": 15300.00
  },
  "revenue": {
    "revenue_this_month": 62000.00,
    "revenue_last_month": 54000.00
  },
  "orders": {
    "pending_orders": 8
  },
  "revenue_by_month": [
    ...
    { "month": "2026-06", "total": 62000.00 }
  ]
}
```
- Todas as queries são paralelizadas via `Promise.all` (5 queries simultâneas)
- `revenue_by_month`: últimos 6 meses com receita realizada (recebíveis pagos)
...
| Método | Rota | Descrição |
|--------|------|-----------|
| GET    | `/v1/suppliers?search=&category=&is_active=&page=&per_page=` | Listar (padrão: apenas ativos) |
| POST   | `/v1/suppliers` | Criar fornecedor |
| GET    | `/v1/suppliers/:id` | Buscar (pix_key mascarada) |
| PATCH  | `/v1/suppliers/:id` | Atualizar |
| DELETE | `/v1/suppliers/:id` | Soft delete (is_active=false) |
| GET    | `/v1/suppliers/:id/payables` | Contas a pagar do fornecedor |
...
```
- `recurrence` padrão: `none` (sem recorrência)
- `recurrence_day`: dia do mês para próxima geração (monthly/quarterly/yearly); ignorado para weekly
- `recurrence_end_date`: última data possível de geração (optional)
