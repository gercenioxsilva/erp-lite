...
2. **Nunca inventar rotas de API.** Todas as rotas autenticadas usam `onRequest: [(fastify as any).authenticate]` e extraem `tenantId` do JWT. Os fluxos de integração entre serviços estão detalhados na seção "Diagramas de Fluxo de Negócio". Rotas existentes:
   - `POST /v1/auth/login` · `POST /v1/auth/register` · `GET /v1/auth/me` · `GET /v1/auth/permissions` — permissões efetivas do usuário logado, RBAC (regra 56), mesmo papel de bootstrap que `GET /v1/tenant/modules` tem hoje
   ...
   - `GET /v1/tenant/modules` · `PATCH /v1/tenant/modules/:key` — módulos opcionais habilitados por tenant (regra 38; `MODULE_KEYS` atuais: `service_orders`, `multi_empresa`, `pos`, `mercadolivre`, `sales_pipeline`); `PATCH` gated por `requirePermission('company', 'manage')` (regra 56)
