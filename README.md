...
82. **Alterar o vencimento de uma conta a receber/pagar já existente — o `PATCH /v1/receivables/:id`/`PATCH /v1/payables/:id` genérico já aceitava `due_date` desde sempre, mas não existia validação nenhuma nem affordance na tela pra usar isso.** Bug real encontrado ao analisar: dava pra mandar qualquer string em `due_date` (nunca validada), inclusive numa conta já paga ou, em recebíveis, numa conta com boleto já emitido no banco — mudar a data ali de nada adiantaria (o boleto real continuaria com o vencimento antigo registrado no Itaú/C6) e deixaria o ERP mentindo pro operador. Duas travas novas, iguais nos dois recursos: nunca em conta `paid` (não faz sentido reagendar o que já foi quitado — mesmo princípio já aplicado ao cancelamento, regra existente em `POST .../cancel`), e a validação de formato via `isValidISODate()` (`lib/dateValidation.ts`, novo — calendário real, rejeita `2026-02-30`, evita que uma string malformada vire erro 500 do Postgres numa coluna `date`). **Só em receivables**: bloqueia também quando `boleto_id` já está setado — não existe fluxo de "alterar boleto já emitido" neste sistema (nunca houve precedente), então a única saída é expirar o boleto primeiro (`PUT /receivables/:id/boleto/expire`, já existente) antes de mudar a data. Payables não têm boleto/banco envolvido, então não há trava equivalente; se a conta for um pagável recorrente (`recurrence != 'none'`), o `due_date` editado passa a ser a nova âncora que `recurringPayablesWorker.ts::generateDueRecurring()` usa pra calcular a próxima ocorrência — comportamento intencional, não um bug, já que o worker sempre lê `due_date` fresco do banco a cada ciclo (nunca cacheia). Frontend: `ReceivablesPage.tsx`/`PayablesPage.tsx` (implementações independentes, sem componente compartilhado) ganham um botão "Alterar vencimento" ao lado do texto de vencimento no drawer de detalhe — some quando a conta não admite mais edição (paga/cancelada/com boleto), reaproveitando o mesmo PATCH que já existia, sem rota nova.

83. **WhatsApp não disparava na autorização de NF-e (0 invocações da Lambda em produção) porque `whatsappAutomationService.ts::dispatch()` sempre foi fire-and-forget e engolia toda falha de elegibilidade (conta não conectada, template não aprovado, cliente sem opt-in, telefone inválido, fila não configurada) num único `console.warn` — nunca visível pro tenant nem pro operador.** `enabled=false` por padrão pra toda automação/tenant (intencional, opt-in) somado a zero sinal de erro é o cenário mais provável: a automação simplesmente nunca foi ligada, e não havia como descobrir isso pela tela. Correção (migration `0093_whatsapp_automation_attempt_status.sql`): 3 colunas novas em `whatsapp_automations` — `last_attempt_at`/`last_attempt_status` (`'sent'|'skipped'`)/`last_skip_reason` (código do `WhatsAppDomainError`, ex. `account_not_connected`) — gravadas a cada tentativa REAL de disparo, isto é, só quando a automação está `enabled` **e** passa da checagem de idempotência (`alreadyDispatched`); o caso "desligada" não precisa de rastro extra porque já é visível pelo próprio `enabled`. `dispatch()` continua nunca lançando pro chamador (`nfeResultsWorker.ts` e os outros gatilhos de negócio não podem quebrar por causa do WhatsApp — mesma filosofia de sempre), só ganhou um `recordDispatchAttempt()` que também nunca lança. Decisão de onde guardar o estado: `whatsapp_messages` foi descartada porque tem `phone_e164 NOT NULL`, incompatível com um "pulado por telefone ausente/inválido"; uma tabela nova seria over-engineering pra um dado que é 1:1 com a automação — por isso entrou direto em `whatsapp_automations`, a mesma linha que já guarda `enabled`/`config`. **Reorganização de UI (o mesmo motivo raiz de "configuração invisível" também explica por que o tenant não achava essas telas)**: Automações/Templates/Mensagens saem do item solto "WhatsApp" no menu Comercial (`/whatsapp`, rota removida) e entram dentro de **Minha Empresa → Integrações** (`WhatsAppSettingsCard.tsx`), junto da conexão da conta Twilio que já morava lá — credencial sensível de tenant não deveria ter navegação própria separada do resto de "Integrações". A tela ganhou um checklist de 3 passos (conta conectada / templates aprovados / automações ativas, com contagem) e um aviso "precisa de atenção" quando alguma automação ligada está sendo pulada — mais o `last_skip_reason` traduzido aparecendo direto no card de cada automação — e um manual embutido colapsável (`WhatsAppManual`, 5 passos) explicando o fluxo Twilio de ponta a ponta, tudo dentro do próprio módulo em vez de documentação externa. Nenhuma rota/endpoint de backend mudou (`/v1/whatsapp/*` intacto, RBAC `whatsapp:view`/`whatsapp:manage` preservado) — só onde a tela mora.
