 ...
 * manages in production. Polls NOTIFICATIONS_QUEUE_URL and calls the same
 * handler function, then deletes successfully processed messages.
 *
 * Used as the Docker Compose entrypoint for the lambda-notifications service:
...
if (!queueUrl) {
  console.error(JSON.stringify({ event: 'runner_config_error', message: 'NOTIFICATIONS_QUEUE_URL is required' }));
  process.exit(1);
}
